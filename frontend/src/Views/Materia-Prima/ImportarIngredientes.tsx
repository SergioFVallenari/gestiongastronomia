...
const ImportarIngredientes: React.FC<ImportarArticulosProps> = ({ onDownloadTemplate }) => {
    ...
    const handleImportar = async () => {
        ...
        try {
            const response = await api.post('/materia_prima/modificarMateriaPrimaMasivo', { url: file, accion: selectedOption });
            ...
            {data && data.length > 0 ? (
                ...
                <Container className="border mt-2 rounded shadow">
                    <Alert variant="info" className="mb-3" hidden={selectedOption !== ''}>
                        <Alert.Heading>Instrucciones para la importación/modificación masiva de ingredientes</Alert.Heading>
                        <ul>
                            <li>-Seleccione la opción deseada en el menú desplegable: "Importación" para agregar nuevos ingredientes o "Modificación" para actualizar precios de ingredientes existentes.</li>
                    ...
                    <Row className="m-2" xs={12} hidden={selectedOption == ''}>
                        <Alert variant="warning">
                            {
                                selectedOption == "2" ? <>
                                    Atención : Esta opción solo modifica precio de costo y cantidad según el SKU del ingrediente.
                                </> : <>
                                    Atención : Esta opción agrega nuevos ingredientes. Asegúrese de eliminar los datos precargados del excel (No eliminar encabezados).
                                </>
                            }
                        </Alert>
                    </Row>
...
export default ImportarIngredientes;